...
const STEPS = [
  {
    ...
    desc: "On your way to the customer",
  },
              ...
              {i < STEPS.length - 1 && (
                <div
                  style={{
                    ...
                    minHeight: 24,
                    margin: "4px 0",
                    backgroundColor: isDone ? COLORS.delivered : COLORS.border,
                  }}
                />
              )}
            </div>
...
export default function ActiveDelivery() {
  ...
  const fetchJobs = useCallback(async () => {
    try {
      ...
      setActiveJob(
        jobs.find((j) => j.statusState === STATUS.OUT_FOR_DELIVERY) || null,
      );
  ...
  useEffect(() => {
    ...
    const onVisible = () =>
      document.visibilityState === "visible" && fetchJobs();
  ...
  const handleMarkDelivered = async () => {
    ...
    try {
      await updateOrderStatus(
        (activeJob._id || activeJob.id) as string,
  ...
  const callCustomer = (phone?: string) => {
    if (!phone) {
      window.alert("This order has no phone number.");
      return;
    }
              ...
              <div>
                ...
                <div style={cardStyle(theme)}>
                  ...
                  >
                    ...
                    >
                      #
                      {String(activeJob._id || activeJob.id)
                        .slice(-5)
                        .toUpperCase()}
                    </span>
                  ...
                  <div style={rowStyle(theme)}>
                    ...
                    <button
                      onClick={() => openMaps(activeJob.deliveryAddress)}
                      style={{
                        background: "none",
                        border: "none",
                        cursor: "pointer",
                ...
                <div style={cardStyle(theme)}>
                  ...
                  <StepIndicator
                    currentStatus={activeJob.statusState}
                    theme={theme}
                  />
                </div>
